"""
Aggregate geometry where monomers touch (no overlap).
...
2. For each subsequent monomer:
   ...
   c. Position monomer above aggregate, binary search to find touching position
   d. Merge monomer into aggregate (boolean union)
"""
...
class AggregateTouching(Geometry):
    """
    Create an aggregate of touching monomers.

    Monomers are positioned using a "drop from above" binary search
    to find the touching position. Each monomer gets a random orientation
    uniformly sampled from SO(3).
    """
    ...
    def __init__(
        ...
        Args:
            ...
            binary_search_steps: Number of binary search iterations for positioning
        """
    ...
    def _translate_until_touching(
        ...
        """
        Binary search to find position where monomer touches aggregate.

        Returns True if valid attachment found.
        """
        ...
        # Binary search
        step_size = (agg_height + monomer_height) / 2
        ...
        for _ in range(self.binary_search_steps):
            ...
            if has_intersection:
                monomer.location.z += step_size  # Move up (away)
            else:
                monomer.location.z -= step_size  # Move down (closer)

            bpy.context.view_layer.update()

        # Final adjustment to ensure contact
        monomer.location.z -= step_size / 2
        bpy.context.view_layer.update()
    ...
    def to_filename(self) -> str:
        ...
        if self.num_monomers is not None:
            return f"aggregate_touching_n{self.num_monomers}{seed_str}_{geom_filename}"
        else:
            diameter_str = f"{self.target_diameter:.1f}".replace(".", "p")
            return f"aggregate_touching_d{diameter_str}{seed_str}_{geom_filename}"
    ...
    def _create_geometry(self) -> bpy.types.Object:
        ...
        while should_continue():
            ...
            if aggregate is None:
                ...
                # Binary search to find touching position
